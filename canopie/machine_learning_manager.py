...
class AnalysisOptionsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.chk_shapefile = QtWidgets.QCheckBox("Also export as Shapefile (.shp)")
        self.chk_shapefile.setToolTip(
            "Export polygons as an ESRI Shapefile alongside the CSV.\n"
            "GeoTIFF images → exact georeferencing from raster transform.\n"
            "EXIF GPS images → approximate georeferencing from flight metadata.\n"
            "Plain images → pixel coordinates (no .prj file)."
        )
        self.chk_shapefile.setChecked(False)

        save_layout.addWidget(self.chk_shapefile, 0, 0, 1, 2)
        save_layout.addWidget(self.chk_custom_save, 1, 0, 1, 2)
        save_layout.addWidget(QtWidgets.QLabel("Save Path:"), 2, 0)
        save_layout.addWidget(self.le_save_path, 2, 1)
        save_layout.addWidget(self.btn_browse_save, 2, 2)
    ...
    def get_options(self):
        ...
        opts = {
            ...
            "export_shapefile": self.chk_shapefile.isChecked(),
        }
        return opts